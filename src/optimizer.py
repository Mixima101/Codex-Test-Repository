...
def _score_row(metrics: dict[str, float], folds_selected: int) -> float:
    ...
    # balance risk-adjusted return, drawdown, turnover drag, and parameter stability
...
def run_two_pass_sweep(
            ...
            for test in test_days_options:
                for cost in txn_cost_options:
                    try:
                        ...
                        focused = SweepConfig(oos, train, test, cost, step_min, step_max, 11, max_min, max_max, 11)
    ...
    valid = summary[summary.get("error").isna()].copy() if "error" in summary.columns else summary
    if valid.empty:
